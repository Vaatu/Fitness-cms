...
// Get the workout template assigned to a client
const getClientWorkoutTemplate = async (req, res) => {
  const { id } = req.params;
  try {
    const client = await Client.findByPk(id, { include: [{ model: WorkoutTemplate }] });
    if (!client) {
      return res.status(404).json({ error: 'Client not found' });
    }
    res.json(client.WorkoutTemplate);
  } catch (error) {
    res.status(500).json({ error: 'Unable to fetch the workout template of the client' });
  }
};
...
const getClientNutritionTemplate = async (req, res) => {
  ...
  try {
    ...
    res.json(client.NutritionTemplate);
  } catch (error) {
